...
while_1: 
    ...
    addiu $sp,$sp,-4          # Grow stack for COMP
    sw $t0,4($sp)             # Save COMP.Right to stack
    lw $t0,-20($fp)           # Load value of: j
    sll $0,$0,0               # NOP: Wait for load delay slot
    lw $t1,4($sp)             # Load COMP.Right to $t1
    addiu $sp,$sp,4           # Shrink stack after COMP
    slt $t0,$t0,$t1           # $t0 = $t0 LT $t1
    beq $t0,$0,endwhile_1     # Evaluate while condition
    sll $0,$0,0               # NOP: Wait for delay slot
                              # START OF COMPARISON
                              # START OF SIGM
    li $t0,1                  # Load literal 1 to $t0
    ...
    lw $t1,-8($fp)            # Load pointer for ARRAY arr to $t1
    sll $0,$0,0               # NOP: Wait for load delay slot
    ...
    lw $t1,4($sp)             # Load COMP.Right to $t1
    addiu $sp,$sp,4           # Shrink stack after COMP
    slt $t0,$t1,$t0           # $t0 = $t0 GT $t1
    beq $t0,$0,endif_2        # Evaluate if condition
    sll $0,$0,0               # NOP: Wait for delay slot
    lw $t0,-20($fp)           # Load value of: j
    sll $0,$0,0               # NOP: Wait for load delay slot
    sll $t0,$t0,2             # Multiply offset by 4
...
endwhile_0: 
    ...
    addiu $sp,$sp,32          # Shrink stack by 32
    jr $ra                    # Jump back
    sll $0,$0,0               # NOP
main: 
    or $fp,$sp,$0             # Start of MAIN. Set fp to top of stack.
    addiu $sp,$sp,-32         # Grow stack by: 32 (8 words)
    sw $ra,($fp)              # Save $ra to offset 0
    li $t0,5                  # Load literal 5 to $t0
    sw $t0,-4($fp)            # Save $t0 to: limit
    li $t0,0                  # Load literal 0 to $t0
    sw $t0,-8($fp)            # Save $t0 to: start
while_3: 
                              # START OF COMPARISON
    lw $t0,-4($fp)            # Load value of: limit
    sll $0,$0,0               # NOP: Wait for load delay slot
    addiu $sp,$sp,-4          # Grow stack for COMP
    sw $t0,4($sp)             # Save COMP.Right to stack
    lw $t0,-8($fp)            # Load value of: start
    sll $0,$0,0               # NOP: Wait for load delay slot
    lw $t1,4($sp)             # Load COMP.Right to $t1
    addiu $sp,$sp,4           # Shrink stack after COMP
    slt $t0,$t0,$t1           # $t0 = $t0 LT $t1
    beq $t0,$0,endwhile_3     # Evaluate while condition
    sll $0,$0,0               # NOP: Wait for delay slot
                              # Start of call: input
    ...
    addiu $sp,$sp,4           # Shrink stack after ASSIGN ARR
    sll $t0,$t0,2             # Multiply offset by 4
    addiu $t2,$fp,-28         # Load address of arr to $t2
    addu $t0,$t2,$t0          # Add $t0 to $t2 to get absolute pointer.
    sw $t1,($t0)              # Save value to address in $t0
                              # START OF SIGM
    li $t0,1                  # Load literal 1 to $t0
    ...
    addu $t0,$t0,$t1          # $t0 = $t0 + $t1
    sw $t0,-8($fp)            # Save $t0 to: start
    j while_3                 # Jump back to while
endwhile_3: 
    sll $0,$0,0               # NOP: Wait for delay slot
                              # Start of call: bubbleSort
    addiu $t0,$fp,-28         # Load address of arr to $t0
...
while_4: 
    ...
    addiu $sp,$sp,4           # Shrink stack after COMP
    slt $t0,$t0,$t1           # $t0 = $t0 LT $t1
    beq $t0,$0,endwhile_4     # Evaluate while condition
    sll $0,$0,0               # NOP: Wait for delay slot
                              # Start of call: output
    lw $t0,-8($fp)            # Load value of: start
    sll $0,$0,0               # NOP: Wait for load delay slot
    sll $t0,$t0,2             # Multiply offset by 4
    addiu $t1,$fp,-28         # Load address of arr to $t1
    addu $t0,$t1,$t0          # Add $t0 to $t1 to get absolute pointer.
    lw $t0,($t0)              # Load value to $t0
    sll $0,$0,0               # NOP: Wait for load delay slot
    sw $t0,-8($sp)            # Save param #0 to $sp-8
